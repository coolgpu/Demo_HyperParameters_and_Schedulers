...
def main():
    ...
    if optim_algo == 'SGD':
        ...
        init_learning_rate = 0.12  # 0.01, 0.12
    ...
    # lr_milestones = []
    # lr_gamma = 1.0  # 1.0 equivalent to no scheduler
    # lr_milestones = [4, 8, 10, 12, 14, 16, 18, 20, 22, 24]
    # lr_gamma = 0.8
    ...
    if flag_write_log:
        logfilename = r'results/' + optim_algo + '_lr{}'.format(init_learning_rate)+'_Epoch{}'.format(epoch)+ '_Schl{:.2f}'.format(lr_gamma) +'_results.log'
        ...
        logstr = 'nsamples={}, batch_size={}, epoch={}, lr={}, lr_gamma={}\ninitial Wa={}, Wb={}, lr_milestone={}\n' \
            .format(nsamples, batch_size, epoch, init_learning_rate, lr_gamma, initWa, initWb, lr_milestones)
    ...
    if flag_plot_final_result:
        ...
        if lr_gamma == 1.0:
            ...
            tmpStr1 = '\nInit '
            tmpStr2 = '\nLR_gamma={}'.format(lr_gamma)+'\nmilestones={}'.format(lr_milestones)
        plt.text(-1.9, 1.9, 'Epoch={}'.format(epoch)+ tmpStr1 + 'Learning Rate={}'.format(init_learning_rate) + tmpStr2,
                 color='black', fontsize=12, ha='left', va='top')

        figfilename = r'results/' + optim_algo + '_lr{}'.format(init_learning_rate) + '_Epoch{}'.format(epoch) + '_Schl{:.2f}'.format(lr_gamma) +  '.png'
