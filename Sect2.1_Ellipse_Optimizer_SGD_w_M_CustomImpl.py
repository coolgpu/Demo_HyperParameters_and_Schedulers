...
def main():
    ...
    batch_size = 64
    epoch = 160
    learning_rate = 0.01 # bad 0.02 #  # 1e-4
    lr_milestones = [5, 7]
    ...
    if flag_log:
        if flag_manual_implement:
            logfilename = 'results/SGD_w_Momentum_custom_implement_LR{}'.format(learning_rate) + '_results.log'
            foutput = open(logfilename, "w")
            foutput.write('SGD_w_Momentum optimization using custom implementation' + '\n')
        else:
            logfilename = 'results/SGD_w_Momentum_custom_implement_LR{}'.format(learning_rate) + '_results.log'
            foutput = open(logfilename, "w")
            foutput.write('SGD_w_Momentum optimization using torch built-in' + '\n')
        ...
        for i_batch, sample_batched in enumerate(xy_dataloader):
            ...
            if flag_manual_implement:  # do the job manually
                ...
                with torch.no_grad():
                    ...
                    if flag_log:
                        mag_dWadWb = math.sqrt(VdWa ** 2 + VdWb ** 2)
                        unitvec_a = VdWa / mag_dWadWb
                        unitvec_b = VdWb / mag_dWadWb
                        ...
                        logstr = ', unitvecWa={:.5f}, unitvecWb={:.5f}, eff_stepsize={:.5f}, ' \
                            ...
                            unitvec_a, unitvec_b, actual_stepsize, dWa, dWb, VdWa, VdWb, 0, 0)
                        foutput.write(logstr)
    ...
    if flag_plot_final_result:
        ...
        plt.text(0.75, 1.5, 'Result of GD w/ Momentum', color='black', fontsize=12)
        plt.show()
