...
def main():
    ...
    learning_rate = 0.01
    lr_milestones = [8]
    lr_gamma = 1  # 1.0 means no LR scheduler
    ...
    if flag_log:
        if flag_manual_implement:
            logfilename = 'results/RMSprop_custom_implement_LR{}'.format(learning_rate) + '_results.log'
            foutput = open(logfilename, "w")
            foutput.write('RMSprop optimization using custom implementation' + '\n')
        else:
            logfilename = 'results/RMSprop_custom_implement_LR{}'.format(learning_rate) + '_results.log'
            foutput = open(logfilename, "w")
            foutput.write('RMSprop optimization using torch built-in' + '\n')
        ...
        for i_batch, sample_batched in enumerate(xy_dataloader):
            ...
            if flag_manual_implement: # do the job manually
                ...
                with torch.no_grad():
                    ...
                    if flag_log:
                        tmp_a = dWa / (math.sqrt(SdWa)+eps)
                        tmp_b = dWb / (math.sqrt(SdWb)+eps)
                        ...
                        logstr = ', unitvecWa={:.5f}, unitvecWb={:.5f}, eff_stepsize={:.5f}, ' \
                            ...
                            unitvec_a, unitvec_b, actual_stepsize, dWa, dWb, 0, 0, math.sqrt(SdWa), math.sqrt(SdWb))
                        foutput.write(logstr)
    ...
    if flag_plot_final_result:
        ...
        plt.text(1.0, 1.5, 'Result of RMSprop', color='black', fontsize=12)
        plt.show()
