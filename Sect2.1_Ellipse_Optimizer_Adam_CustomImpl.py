...
def main():
    ...
    if flag_log:
        if flag_manual_implement:
            logfilename = 'results/Adam_custom_implement_LR{}'.format(learning_rate) + '_results.log'
        ...
        for i_batch, sample_batched in enumerate(xy_dataloader):
            ...
            if flag_manual_implement: # do the job manually
                ...
                with torch.no_grad():
                    ...
                    if flag_log:
                        tmp_a = (VdWa / beta1_correction) / (torch.sqrt(SdWa) / math.sqrt(beta2_correction) + eps)
                        tmp_b = (VdWb / beta1_correction) / (torch.sqrt(SdWb) / math.sqrt(beta2_correction) + eps)
